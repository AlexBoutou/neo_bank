...
FROM
    (
        SELECT
            ...
            date_diff(parse_datetime("%Y, %m", um.month_year), EXTRACT(datetime
        FROM
            us.created_date), MONTH) AS diff_with_creation_month,
            us.created_date as created_date
        FROM
            user_cross_month um
            LEFT JOIN {{ ref("int_users_with_transactions_month") }}
            ut
            ON um.user_id = ut.user_id
            AND um.month_year = ut.month_year
